...
(defun quail-conv-get-trans-and-map (alist)
  ...
  (let* ((ch (char-after (+ quail-conv-offset (point))))
    ...
    (when (consp trans)
      (let* ((next-char (char-after (+ 1 quail-conv-offset (point))))
             (index (cond
                     ((member next-char (list ?1 ?2 ?3 ?4 ?5 ?6 ?7 ?8 ?9))
                      (incf quail-conv-offset)
                      (1- (string-to-number (char-to-string next-char))))
                     ((equal ?0 next-char)
                      (incf quail-conv-offset)
                      9)
                     (t
                      0))))
        (setq trans (aref (cdr trans) index))))
